<template>
  <div class="d-flex align-center justify-center h-screen">
    <v-card class="mx-auto w-100" max-width="454">
      <v-card-title class="text-h5 bg-primary text-white py-4">
        Administrador
      </v-card-title>
        ...
        <v-form @submit.prevent="submit" class="mt-7">
          ...
          <div class="mb-6">
            <v-text-field
              ...
              color="primary"
              hide-details="auto"
              :type="showEveOutline ? 'text' : 'password'"
              class="mt-2"
            >
              <template v-slot:append-inner>
                <v-icon
                  :icon="
                    showEveOutline ? 'mdi-eye-outline' : 'mdi-eye-off-outline'
                  "
                  class="cursor-pointer"
                  @click="showEveOutline = !showEveOutline"
                ></v-icon>
              </template>
            </v-text-field>
          </div>
          ...
          ></v-alert>
          <v-btn
            :loading="fetchLogin?.loading"
            :readonly="fetchLogin?.loading"
            color="primary"
            class="w-100 pa-1"
            size="large"
            text="Login"
